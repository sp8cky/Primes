...
def optimized_ramzy_test_protocoll(n: int, seed: Optional[int] = None) -> bool: 
    if n <= 1: 
        test_data["Optimized Ramzy"][n]["result"] = INVALID
        return INVALID
    
    decomposition = helpers.find_ramzy_decomposition(n)
    if decomposition is None: 
        test_data["Optimized Ramzy"][n]["result"] = NOT_APPLICABLE
        test_data["Optimized Ramzy"][n]["reason"] = "Keine Zerlegung N=K*p^n+1 gefunden"
        return NOT_APPLICABLE
    ...
    if not found_valid_j: 
        test_data["Optimized Ramzy"][n]["result"] = NOT_APPLICABLE
        test_data["Optimized Ramzy"][n]["reason"] = "Kein gültiges j gefunden"
        return NOT_APPLICABLE
    ...
    for a in range(2, n):
        ...
        if cond1 and cond2:
            test_data["Optimized Ramzy"][n]["a_values"] = [(a, cond1, cond2)]
            test_data["Optimized Ramzy"][n]["result"] = PRIME
            return PRIME
    
    test_data["Optimized Ramzy"][n]["result"] = COMPOSITE
    test_data["Optimized Ramzy"][n]["reason"] = "Kein geeignetes (a,j)-Paar gefunden"
    return COMPOSITE